...
personRoutes.route('/client').get(function (req, res) {
  Person.find(function (err, persons){
    if(persons) {
      persons = persons.filter(t => t.isActive == true);
      res.json(persons);
    }
    else {
      console.log(err);
...
personRoutes.route('/update/avail/:id').post(function (req, res) {
  Person.findById(req.params.id, function(err, person) {
    if (!person)
      return next(new Error('Could not load Document'));
    else {
      person.isActive = req.body.isActive;
      person.save().then(person => {
        res.json('Update complete');
      })
        .catch(err => {
